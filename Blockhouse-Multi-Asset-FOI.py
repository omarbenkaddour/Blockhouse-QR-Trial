...
# -------------------------------------------------------------------
# 3.  Best‑level OFI (depth‑0)  – per‑symbol, per‑minute
# -------------------------------------------------------------------
# aggregate depth‑0 order‑flow that we already created in OF_bid_all / OF_ask_all
agg_bid0 = (OF_bid_all.groupby(['symbol','minute'])['OF_bid_00']
                        .sum().rename('bid0_of').reset_index())
agg_ask0 = (OF_ask_all.groupby(['symbol','minute'])['OF_ask_00']
                        .sum().rename('ask0_of').reset_index())

best_level_df = (agg_bid0.merge(agg_ask0, on=['symbol','minute'], how='outer')
                           .fillna(0.0))
best_level_df['ofi_best_level'] = best_level_df['bid0_of'] - best_level_df['ask0_of']

# -------------------------------------------------------------------
# 4. Multi‑level, depth‑scaled OFI per (symbol, minute)
# -------------------------------------------------------------------
events_parts = []
...
# -------------------------------------------------------------------
# 5. “Integrated” OFI  (1st PC across levels)  – per‑symbol
# -------------------------------------------------------------------
integrated_parts = []
...
# -------------------------------------------------------------------
# 6. Cross‑section PCA  (market‑wide OFI factor)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 7. Cross-Asset OFI + Indiosyncratic OFI
# -------------------------------------------------------------------
